...
def infos(atlet, event_name):
    ...
    for i in pos['positions']:         # [{'y': 2, 'x':3}, {'y': 20, 'x':3}] 
            x_pos.append(i[0]['x'])     # I collect the x in the list X_pos and y in the list Y_pos
            x_pos.append(i[1]['x'])
            y_pos.append(i[0]['y'])
            y_pos.append(i[1]['y'])
...
draw_pitch("#195905","black","horizontal","full")
sns.kdeplot(messi_pos[0],messi_pos[1], shade = "True", color = "green", n_levels = 30,zorder=2)

plt.gca().invert_yaxis() # to invert respext to the y axis. This correspind to messi's real movements.
...
draw_pitch("#195905","black","horizontal","full")
sns.kdeplot(ronaldo_pos[0],ronaldo_pos[1], shade = "True", color = "green", n_levels = 30,zorder=2)
plt.gca().invert_yaxis()
